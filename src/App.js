...
class App extends Component {
  state = {
    tabsListId: tabsList[0].tabId,
    score: 0,
    time: 60,
    imageLink: imagesList[0].imageUrl,
    isCond: false,
  }
  ...
  onClickTabsList = tabId => {
    this.setState({tabsListId: tabId})
  }

  getImagesList = () => {
    const {tabsListId} = this.state

    const updateImagesList = imagesList.filter(
      eachItem => eachItem.category === tabsListId,
    )

    return updateImagesList
  }

  ChangeImages = thumbnailUrl => {
    const {imageLink} = this.state
    const filterImage = imagesList.filter(
      eachId => eachId.thumbnailUrl === thumbnailUrl,
    )
    const {imageUrl} = filterImage[0]
  ...
  render() {
    const UpdateList = this.getImagesList()
    const {tabsListId, score, time, imageLink, isCond} = this.state
        ...
        <nav className="navbar">
          ...
          <ul className="score-time-container">
            <li className="score-text">
              <p>
                Score: <span className="running-score">{score}</span>
              </p>
            </li>
            <li className="time-list">
              ...
              />
              <p className="running-score">{time} sec</p>
            </li>
          </ul>
        </nav>
        {isCond ? (
          ...
          <div className="responsive-container">
            <img src={imageLink} alt="match" className="match-image" />
            <ul className="list-order-tab">
              {tabsList.map(eachTab => (
                <li key={eachTab.tabId} className="list-tab-container">
                  <button
                    type="button"
                    className={
                      eachTab.tabId === tabsListId
                        ? 'click-color'
                        : 'tab-button'
                    }
                    onClick={() => this.onClickTabsList(eachTab.tabId)}
                  >
            ...
            <ul className="list-order-images">
              {UpdateList.map(eachImage => (
                <li key={eachImage.id} className="list-images-container">
                  <button
                    type="button"
                    className="images-btn"
                    onClick={() => this.ChangeImages(eachImage.thumbnailUrl)}
                  >
                    <img
